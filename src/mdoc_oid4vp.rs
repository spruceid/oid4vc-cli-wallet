...
pub async fn handle_request(request: Url) -> Result<()> {
    info!("generating mDL and key...");
    let wallet = super::wallet::generate_credential();

    info!("validating authorization request...");
    ...
    info!("signing and encrypting response...");
    let jwe = finish(wallet.key, prepared_response, state).await?;

    info!("POSTing response to {response_uri}...");
    let redirect = send(response_uri, jwe).await?;

    info!("SUCCESS! Redirecting to {redirect}");
...
fn validate_authz_request(request: Url) -> Result<AuthorizationRequest> {
    ...
    if scheme != SCHEME {
        warn!("Request scheme was invalid: '{scheme}'")
    }
    ...
    if client_id.is_none() {
        warn!("Authorization Request did not contain 'client_id'");
    }
...
fn validate_cis_x509_san_uri(client_id: &str, jwt: &str) -> Result<()> {
    ...
    if leaf_cert.tbs_certificate.get::<SubjectAltName>() == Ok(None) {
        warn!("Missing SubjectAlternativeName in x509 cert.");
        if !leaf_cert
            .tbs_certificate
            .subject
            .0
        ...
        .filter_map(|r| match r {
            ...
            Err(e) => {
                warn!("Unable to parse SubjectAlternativeName from DER: {e}");
                None
            }
        })
        .flatten()
...
async fn get_request_object(request_uri: Url) -> Result<RequestObject> {
    ...
    info!("client_id_scheme: {client_id_scheme:?}");

    match client_id_scheme {
        s if s == REDIRECT_URI => validate_cis_request_uri(&request)?,
        s if s == X509_SAN_DNS => validate_cis_x509_san_dns(&request.client_id, &body)?,
        s if s == X509_SAN_URI => validate_cis_x509_san_uri(&request.client_id, &body)?,
    ...
    let jwk = keys
        ...
        .filter_map(|v| {
            ...
            match jwk {
                Ok(jwk) => Some(jwk),
                Err(e) => {
                    warn!("unable to parse a JWK in keyset: {e}");
                    None
                }
            }
        })
        .find(|jwk| {
            let Some(crv) = jwk.curve() else {
                warn!("jwk in keyset was missing 'crv'");
                return false;
            };
...
fn parse_request(
    ...
    if let Some(id) = client_id {
        if req.client_id != id {
            warn!("client_id from AuthorizationRequest did not match client_id in request object");
        }
    }
